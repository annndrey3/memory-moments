...
> **Конфіг сайту в БД:** Telegram-токен, знижки на фотодрук, контакти, доставка, банер/SEO та
> SFTP-сховище також редагуються в адмінці (**Сайт**, право `settings.system`) і зберігаються в
> таблиці `settings`. Значення з БД мають пріоритет, дефолти — в коді (`utils/siteConfig.js`),
> `.env` — резервний відкат. Окремої міграції не потрібно (БД зберігає лише перевизначення).
...
**Сайт** (`/admin/site`, право `settings.system`) — «Налаштування сайту», власник редагує без розробника
(`client/src/pages/admin/AdminSiteConfigPage.jsx`):
- **Контакти та філії** — телефон, Instagram, Telegram, Viber, адреса, посилання на карти, список філій, години роботи
- **Доставка** — методи доставки + тоглі ввімкнення + список філій самовивозу
- **Знижки на фотодрук** — рівні знижок за кількістю
- **Головний банер** (заголовок/слоган) + **SEO** (назва сайту, meta description)
- **Telegram** — токен бота (маскується), chat id + кнопка «надіслати тест»
- **Сховище фото (SFTP)** — host/port/user/password (маскується)/віддалений шлях + увімкнення + «Перевірити з'єднання»
...
# Конфіг сайту (право settings.system)
GET    /api/admin/settings/site-config                весь конфіг для адмінки
PUT    /api/admin/settings/site-config/:section       зберегти секцію
POST   /api/admin/settings/site-config/telegram/test  тест Telegram
GET/PUT /api/admin/settings/storage                   конфіг SFTP-сховища (пароль маскується)
POST   /api/admin/settings/storage/test               перевірка SFTP-з'єднання
```
...
## Доставка фото дизайнеру (SFTP) + Telegram

Фото покупців **завжди** лежать на VPS (джерело істини). Додатково в адмінці (**Сайт → Сховище фото**)
налаштовується **SFTP-призначення**, щоб доставляти файли замовлення на ПК/сервер дизайнера:

- Фоновий воркер-ретрай **кожні 10 хв** + негайна спроба при створенні замовлення. Якщо призначення

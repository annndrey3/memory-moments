...
в фоне), `photo_delivery_status`/`photo_delivery_at`/`photo_delivery_attempts` (фоновая доставка
фото на SFTP-хранилище дизайнера). Макет и метаданные позиции (fabric JSON, URL print-файлов,
`innerPhotos` книги и `book`-мета) хранятся в `order_items.design_data` (JSON).
...
и чекауте; таблица — на странице `/prices`. Уровни скидок теперь **редактируются в админке**
(Налаштування сайту → «Знижки на фотодрук»), значения берутся из `settings` с откатом к кодовым
...
- **Онбординг-тур** — пошаговый гид с подсветкой для новых посетителей
  (`src/components/DesignerTour.jsx`): 5 шагов, кнопка «Пропустити навчання», показывается один раз
  (localStorage `mm_designer_tour_v1`); плавающая кнопка «Підказки» повторяет тур.
...
**Last Updated:** 2026-06-19 · **Status:** 🟢 Интеграция + фотокниги (Slim/Print Book, предпросмотр, ZIP-печать), рамки/коллаж/шрифты + тур в конструкторе, админка «Налаштування сайту», SFTP-доставка фото, скидки на фотопечать, аудит-хардненинг и ops

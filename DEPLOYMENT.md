...
> также править в админке (**Налаштування сайту**, право `settings.system`) — значения хранятся в
> таблице `settings` и имеют приоритет над кодовыми значениями / `.env`. Отдельная миграция не нужна.
...
## 8a. Доставка фото дизайнеру по SFTP (опционально)

Фото покупателей всегда хранятся на VPS. Чтобы дополнительно автоматически доставлять файлы
заказов на ПК/сервер дизайнера по SFTP:

1. Запустите **SFTP-сервер** на ПК дизайнера (например, OpenSSH/WinSCP-сервер).
2. Пробросьте порт на роутере (port-forward) или используйте DDNS, чтобы VPS мог достучаться
   до этого ПК (даже за NAT).
3. В админке: **Налаштування сайту → Сховище фото** — заполните host/port/user/password и
   удалённый путь, включите доставку и нажмите **«Перевірити з'єднання»**.

Дальше работает фоновый воркер: при создании заказа делается немедленная попытка, затем повтор
каждые 10 минут. Если ПК дизайнера выключен (например, ночью) — заказ остаётся `pending` и
доставится, как только адрес станет доступен. При заказе с **>3 фото** в Telegram уходит ссылка
на скачивание (`PUBLIC_URL` + путь), а не вложения.
...
- [ ] (Опц.) `PUBLIC_URL` задан, если используете Telegram-ссылку на скачивание при заказах с >3 фото
- [ ] (Опц.) SFTP-хранилище: пароль вводится в админке (хранится в `settings`, маскируется), соединение проверено кнопкой
